...
public final class NeoForgeUpdateScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.centeredText(this.font, this.title, centerX, y, 0xFFFFFF);
        graphics.centeredText(this.font, Component.literal("当前版本: " + currentVersion), centerX, y + 24, 0xCCCCCC);
        graphics.centeredText(this.font, statusLine(result), centerX, y + 42, statusColor(result));
        if (result.status() == BakaPotatoUpdateChecker.Status.READY) {
            graphics.centeredText(this.font, Component.literal("最新版本: " + result.latestVersion() + " / reminds=" + result.reminds()), centerX, y + 60, 0xCCCCCC);
            if (result.shouldRemind()) {
                graphics.centeredText(this.font, Component.literal("建议前往下载最新版本。"), centerX, y + 78, 0xFFFF55);
            }
        }
    }
